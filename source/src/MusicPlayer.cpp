...
MusicPlayer::MusicPlayer() : globalVolume(5) {
  if (Mix_OpenAudio(44100, MIX_DEFAULT_FORMAT, 2, 2048) < 0) {
    throw std::runtime_error("SDL_mixer could not initialize! Error: " +
...
void MusicPlayer::playSound(const std::string &soundPath, int loops) {
  if (soundEffects.find(soundPath) == soundEffects.end()) {
    ...
    if (!sound) {
      throw std::runtime_error("Failed to load sound effect! Error: " +
                               std::string(Mix_GetError()));
    }
  ...
  if (Mix_PlayChannel(channel, soundEffects[soundPath], loops) == -1) {
    throw std::runtime_error("Failed to play sound effect! Error: " +
                             std::string(Mix_GetError()));
  }
